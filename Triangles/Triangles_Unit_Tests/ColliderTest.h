...
TEST(ColliderCreation, ColliderTest) {
	...
	Vector::Vector3<double> a(0.2, 4, 0.88821312), b(1, 1, 1), c(56, 321, 423746.23463284);
	std::vector<Vector::Vector3<double>> points = {a, b, c};
...
TEST(ColliderReInit, ColliderTest) {
	...
	std::vector<Vector::Vector3<double>> lots_of_points;
	...
	for (size_t i = 0; i < 1000; i++)
		ASSERT_EQ(col1.GetPoint(i), Vector::Vector3<double>((double)i, 2 * (float)i, 3 * (float)i ));
}

TEST(OnePlaneTrue, ColliderTest)
{
	std::vector<Vector::Vector3<double>> points_a, points_b;
